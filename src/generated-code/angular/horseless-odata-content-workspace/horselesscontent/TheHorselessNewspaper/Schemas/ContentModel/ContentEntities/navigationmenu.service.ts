...
export class NavigationMenuService extends ODataEntitySetService<NavigationMenu> {
  ...
  public accessControlEntries(key: EntityKey<NavigationMenu>): ODataNavigationPropertyResource<AccessControlEntry> { 
    return this.entity(key).navigationProperty<AccessControlEntry>('AccessControlEntries'); 
  }
  public fetchAccessControlEntries(key: EntityKey<NavigationMenu>, options?: ODataQueryArgumentsOptions<AccessControlEntry>) {
  ...
  public addAccessControlEntryToAccessControlEntries(key: EntityKey<NavigationMenu>, target: ODataEntityResource<ODataEntities<AccessControlEntry>>, {etag}: {etag?: string} = {}): Observable<any> {
    return this.accessControlEntries(key).reference()
      .add(target);
  }
  public removeAccessControlEntryFromAccessControlEntries(key: EntityKey<NavigationMenu>, {target, etag}: {target?: ODataEntityResource<ODataEntities<AccessControlEntry>>, etag?: string} = {}): Observable<any> {
    return this.accessControlEntries(key).reference()
      .remove(target);
  }
