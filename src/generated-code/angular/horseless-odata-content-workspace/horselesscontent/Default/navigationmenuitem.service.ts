...
export class NavigationMenuItemService extends ODataEntitySetService<NavigationMenuItem> {
  ...
  public accessControlList(key: EntityKey<NavigationMenuItem>): ODataNavigationPropertyResource<AccessControlEntry> { 
    return this.entity(key).navigationProperty<AccessControlEntry>('AccessControlList'); 
  }
  public fetchAccessControlList(key: EntityKey<NavigationMenuItem>, options?: ODataQueryArgumentsOptions<AccessControlEntry>) {
  ...
  public addAccessControlEntryToAccessControlList(key: EntityKey<NavigationMenuItem>, target: ODataEntityResource<ODataEntities<AccessControlEntry>>, {etag}: {etag?: string} = {}): Observable<any> {
    return this.accessControlList(key).reference()
      .add(target);
  }
  public removeAccessControlEntryFromAccessControlList(key: EntityKey<NavigationMenuItem>, {target, etag}: {target?: ODataEntityResource<ODataEntities<AccessControlEntry>>, etag?: string} = {}): Observable<any> {
    return this.accessControlList(key).reference()
      .remove(target);
  }
