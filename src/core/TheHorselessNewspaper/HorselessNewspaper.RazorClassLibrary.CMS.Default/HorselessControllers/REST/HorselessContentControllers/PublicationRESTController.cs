...
{
    [ApiController]
    [Route("{__tenant__}/" + RESTContentModelControllerStrings.API_HORSELESSCONTENTMODEL_PUBLICATION)]
    public class PublicationRESTController : ControllerBase,
        IRESTContentController<Publication>
    {
        public IContentCollectionService<IQueryableContentModelOperator<Publication>, Publication> _contentCollectionService { get; set; }
        public ITenantInfo CurrentTenant { get; set; }

        public PublicationRESTController(IContentCollectionService<IQueryableContentModelOperator<Publication>,
                Publication> contentCollectionService, ITenantInfo tenantInfo)
        {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(Publication))]
        [ProducesResponseType(StatusCodes.Status201Created, Type = typeof(Publication))]
        public async Task<ActionResult<Publication>> Create([FromBody] Publication contentCollection)
        {
        ...
        public async Task<ActionResult<Publication>> GetByObjectId([FromRoute] string objectId)
        {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(Publication))]
        [ProducesResponseType(StatusCodes.Status202Accepted, Type = typeof(Publication))]
        public async Task<ActionResult<Publication>> Update([FromRoute] string contentCollectionId, [FromBody] Publication contentCollection)
        {
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(IEnumerable<ContentModel.Publication>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<IEnumerable<ContentModel.Publication>>> GetByPageNumber(int pageSize = 10, int pageNumber = 1, int pageCount = 1)
        {
