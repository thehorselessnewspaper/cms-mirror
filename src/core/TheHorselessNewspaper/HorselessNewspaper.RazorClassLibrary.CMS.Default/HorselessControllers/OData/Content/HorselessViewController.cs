...
{
    [Route("ODataContent/HorselessView")]
        ...
        ODataController, IContentQueryController<ContentModel.HorselessView>
    {
        ...
        public async Task<ActionResult<IQueryable<ContentModel.HorselessView>>> Get(ODataQueryOptions<ContentModel.HorselessView> options)
        {
            var isFailedAuthorization = false;

            var potentialResult = await _horselessViewService.Query(options);
